...
FlyngFoogaFoogRed::FlyngFoogaFoogRed(sf::Vector2f pos)
    ...
    , m_fooga75Loaded(false)
{
    ...
    m_fooga25Loaded = m_fooga25Texture.loadFromFile("assets/sprites/fooga_red_25.png");
    m_fooga50Loaded = m_fooga50Texture.loadFromFile("assets/sprites/fooga_red_50.png");
    m_fooga75Loaded = m_fooga75Texture.loadFromFile("assets/sprites/fooga_red_75.png");

    // Flying animation
    m_flyLoaded = true;
    for (int i = 0; i < 3; ++i) {
        std::string path = "assets/sprites/fooga_fly_red_frame"
                         + std::to_string(i + 1) + ".png";
        if (!m_flyTextures[i].loadFromFile(path)) {
            std::cerr << "[FlyngFoogaFoogRed] Missing: " << path << "\n";
            m_flyLoaded = false;
        }
    }
}
